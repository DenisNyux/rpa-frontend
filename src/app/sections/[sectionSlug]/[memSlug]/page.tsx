...
async function MemberPage({ params }: MemberPageProps) {
  ...
  return <div className="mt-6 mb-12 mx-20 lg:mx-12 xs:mx-8">
    ...
    <Link href={`/sections/${params.sectionSlug}/`} className=" text-[#5E050D] text-base leading-5">← Назад</Link>
    
    <h2 className="mt-6">{member.name}</h2>
    {sectTitle && <span className="text-base">Руководитель секции {sectSlug ? <Link href={`/sections/${sectSlug}`} className="underline text-[#5e050d]">{sectTitle}</Link> : sectTitle}</span>}
