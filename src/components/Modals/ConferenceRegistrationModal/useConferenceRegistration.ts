...
export const useConferenceRegistration = (
  ...
  const [submitMessage, setSubmitMessage] = useState<FormState["submitMessage"]>(null);
  ...
  const handleSubmit = useCallback(async (e: React.FormEvent) => {
    ...
    try {
      ...
      const recaptchaToken = await executeRecaptcha(FORM_CONFIG.RECAPTCHA_ACTION);
      ...
      setSubmitMessage(createSuccessMessage(MESSAGES.SUCCESS.REGISTRATION_SUCCESS));
   ...
   */
  const renderField = useCallback(({ name, config }: RenderFieldParams) => {
    const hasError = hasFieldError(name, touched, errors);
    const hasSuccess = hasFieldSuccess(name, touched, errors, formData);
    const fieldClasses = createFieldClasses(hasError, hasSuccess, "formControl");
